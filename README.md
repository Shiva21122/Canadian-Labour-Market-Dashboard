...
How have unemployment, employment, and participation rates evolved across Canada's 10 provinces from 2015 to 2026 — including the COVID-19 shock and recovery — and which provinces are outperforming?
...
```
canadian-labour-market-pipeline/
│
├── data/
│   ├── raw/                       # StatCan full extract + metadata (1.1 GB - not in GitHub)
│   └── processed/
...
│   └── adtest.py                  # Ad-hoc data checks
│
├── dashboards/
│   └── Canadian_Labour_Market_Dashboard.pbix
│
├── industry-employment/           # Sub-project: employment by industry (Table 14-10-0355-01)
│   ├── data/                      # Raw (139 MB - not in GitHub) + cleaned industry data
│   ├── scripts/                   # clean / load / analysis / peek scripts
│   └── Industry_Employment_Dashboard.pbix
│
└── README.md
```

> **Note:** the raw StatCan extracts exceed GitHub's 100 MB limit and are excluded via `.gitignore` — the download link is in the Data Source section below.
...
3. Download the raw data from Statistics Canada:
   ...
   - Place the extracted CSV in the `data/raw/` folder as `14100287.csv`
...
5. Open `dashboards/Canadian_Labour_Market_Dashboard.pbix` in Power BI Desktop
